...
def make_bottleneck_stage(in_ch, out_ch, num_blocks=2, use_eca=False, use_sa=False):
...
def make_temporal_stage(in_ch, out_ch, num_blocks=2, n_segment=2, n_div=8, use_sa=False):
...
class SKD_TSTSAN_v2(nn.Module):
    def __init__(self, num_classes=5, amp_factor=5, n_segment_t=2):
        super().__init__()
    ...
    def forward(self, input: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # 动态初始化 T 分支 Encoder 与 Stem
        if self.Aug_Encoder_T is None:
            from motion_magnification_learning_based_master.magnet import Encoder_No_texture as MagEncoder_No_texture
            self.Aug_Encoder_T = MagEncoder_No_texture(dim_in=t_ch).to(device)
        if self.stem_T is None:
            self.stem_T = nn.Sequential(
                nn.Conv2d(t_ch, 64, 5, stride=1, padding=2, bias=False),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True)
            ).to(device)
        if self.ac1_T is None:
            self.ac1_T = make_temporal_stage(64, 128, num_blocks=2, n_segment=self.n_segment_t, use_sa=True).to(device)
        if self.mid_T is None:
            self.mid_T = nn.Sequential(
                TemporalShift(Bottleneck(64, 64), n_segment=self.n_segment_t),
                TemporalShift(Bottleneck(64, 64), n_segment=self.n_segment_t),
                nn.AvgPool2d(3, stride=2, padding=1)
            ).to(device)
        if self.ac2_T is None:
            self.ac2_T = make_temporal_stage(64, 128, num_blocks=2, n_segment=self.n_segment_t).to(device)
        if self.final_T is None:
            self.final_T = make_temporal_stage(64, 128, num_blocks=2, n_segment=self.n_segment_t).to(device)

        # Augmentation / motion manipulation
        x1_onset = torch.zeros_like(x1)
        x2_onset = torch.zeros_like(x2)
        ...
        ac1_feat = torch.cat([
            self._global_pool_flat(self.ac1_pool(ac1_x1)),
            self._global_pool_flat(self.ac1_pool(ac1_x2)),
            self._global_pool_flat(self.ac1_pool(ac1_x3))
        ], dim=1)
        ...
        ac2_feat = torch.cat([
            self._global_pool_flat(self.ac2_pool(ac2_x1)),
            self._global_pool_flat(self.ac2_pool(ac2_x2)),
            self._global_pool_flat(self.ac2_pool(ac2_x3))
        ], dim=1)
        ...
        final_feat = torch.cat([
            self._global_pool_flat(self.final_pool(f1)),
            self._global_pool_flat(self.final_pool(f2)),
            self._global_pool_flat(self.final_pool(f3))
        ], dim=1)
